...
1. 栈(Stack)：存放的是方法中的局部变量和被调用方法参数，局部变量一旦超出作用域会立刻从栈内存中消失，方法在栈中运行
...
1. 包(package)
   ...
   1. package语句
      ...
      要将某个类放入包中，余姚在定义类时使用package语句指明属于哪个包
...
2. this关键字的另一个作用是在一个构造方法中调用该类的另一个构造方法，此时this语句必须是调用方法体中的第一条语句
3. this关键字只能用在非static方法中，实际上，在对象调用一个非static方法时向方法传递了一个引用，这个引用就是对象本身
